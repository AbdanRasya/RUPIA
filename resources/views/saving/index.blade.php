...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav-item { display: flex; align-items: center; gap: 0.75rem; padding: 0.7rem 0.75rem; border-radius: var(--radius-sm); color: var(--sidebar-text); text-decoration: none; font-size: 0.875rem; font-weight: 500; transition: all 0.18s ease; margin-bottom: 2px; cursor: pointer; border: none; background: none; width: 100%; text-align: left; }
        ...
        /* ── PANEL ── */
        .panel { background: var(--card-bg); border: 1px solid var(--border-color); border-radius: var(--radius-md); padding: 1.5rem; margin-bottom: 1.25rem; box-shadow: var(--shadow-sm); }

        /* ── SAVING CARDS ── */
        .saving-card { padding-bottom: 1.5rem; margin-bottom: 1.5rem; border-bottom: 1px solid var(--border-color); }
        .saving-card:last-child { border-bottom: none; padding-bottom: 0; margin-bottom: 0; }
        .progress-container { background: var(--bg-page); height: 6px; border-radius: 10px; margin: 0.875rem 0; overflow: hidden; }
        .progress-bar { height: 100%; background: linear-gradient(90deg, var(--primary), #0077B6); border-radius: 10px; transition: 0.6s cubic-bezier(0.4, 0, 0.2, 1); }

        /* ── BUTTONS ── */
        .btn { padding: 0.5rem 1rem; border-radius: var(--radius-sm); font-weight: 600; font-size: 0.8rem; cursor: pointer; border: none; transition: 0.18s; text-decoration: none; display: inline-block; }
        ...
        /* ── PROMO CARD ── */
        .promo-card {
            background: linear-gradient(135deg, #00A550, #0077B6);
        ...
        .promo-card::before { content:''; position:absolute; top:-30px; right:-30px; width:120px; height:120px; border-radius:50%; background:rgba(255,255,255,0.08); }
        ...
        .modal-input { width: 100%; padding: 0.75rem 1rem; border-radius: var(--radius-sm); border: 1px solid var(--border-color); background: var(--bg-page); color: var(--text-main); margin-bottom: 0.875rem; outline: none; font-size: 0.9rem; font-family: 'Inter', sans-serif; transition: 0.2s; }
...
<body>
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="{{ url('/saving') }}" class="nav-item active">
    ...
    <div class="main-area">
        <header class="topbar">
            <div class="topbar-left">
                ...
                <p>Kelola impian masa depanmu secara terukur</p>
            </div>
                ...
                <div class="panel">
                    @forelse($savings ?? [] as $saving)
                        ...
                        <div class="saving-card">
                            <div style="display:flex;justify-content:space-between;align-items:flex-end;">
                                ...
                                <span style="font-weight:700;color:var(--primary);font-size:0.9rem;">{{ $progress }}%</span>
                            </div>
                            <div class="progress-container">
                                <div class="progress-bar" style="width: {{ $progress > 100 ? 100 : $progress }}%;"></div>
                            </div>
                            <div style="display:flex;justify-content:space-between;align-items:center;">
                                <p style="font-size:0.85rem;color:var(--text-muted);">Terkumpul: <b style="color:var(--text-main);">Rp {{ number_format($saving->current_amount, 0, ',', '.') }}</b></p>
                    ...
                    @empty
                        <div style="text-align:center;padding:2rem 0;">
                            ...
                            <p style="color:var(--text-muted);font-size:0.9rem;">Belum ada target yang dibuat.</p>
                        </div>
                    @endforelse
                </div>

                <aside>
                    <div class="promo-card" style="margin-bottom:1.25rem;">
                        <p style="font-size:0.7rem;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:rgba(255,255,255,0.7);margin-bottom:0.5rem;position:relative;z-index:1;">Fitur</p>
                        <h3 style="font-size:1rem;font-weight:700;margin-bottom:0.5rem;position:relative;z-index:1;">Tambah Target Baru</h3>
                        <p style="font-size:0.8rem;opacity:0.8;margin-bottom:1.25rem;position:relative;z-index:1;">Wujudkan barang impianmu dengan menabung konsisten.</p>
                        <button class="btn" style="background:#fff;color:var(--primary-dark);width:100%;position:relative;z-index:1;" onclick="document.getElementById('goalModal').style.display='flex'">+ Buat Goal Baru</button>
                    </div>
                </aside>
            </div>
        </main>
    ...
    <div id="goalModal" class="modal">
        <form class="modal-content" action="{{ url('/saving/store') }}" method="POST">
            @csrf
            <h2 style="font-size:1.1rem;font-weight:700;margin-bottom:1.25rem;">Goal Baru</h2>
            ...
            <button type="submit" class="btn btn-primary" style="width:100%;padding:0.75rem;">Pasang Target</button>
            <button type="button" class="btn btn-outline" style="width:100%;margin-top:0.5rem;padding:0.75rem;" onclick="document.getElementById('goalModal').style.display='none'">Batal</button>
        </form>
    </div>
    ...
    <div id="depositModal" class="modal">
        <form id="depositForm" class="modal-content" method="POST">
            @csrf
            <h2 style="font-size:1.1rem;font-weight:700;margin-bottom:1.25rem;">Isi Celengan</h2>
            <p id="depositTitle" style="font-size:0.875rem;color:var(--text-muted);margin-bottom:1rem;"></p>
            <input type="number" name="amount" class="modal-input" placeholder="Nominal Nabung (Rp)" required>
            <button type="submit" class="btn btn-primary" style="width:100%;padding:0.75rem;">Nabung Sekarang</button>
            <button type="button" class="btn btn-outline" style="width:100%;margin-top:0.5rem;padding:0.75rem;" onclick="document.getElementById('depositModal').style.display='none'">Batal</button>
        </form>
    </div>
    ...
    <script>
        ...
        function openDepositModal(id, title) { 
            document.getElementById('depositTitle').innerText = 'Target: ' + title;
